...
def moving_average_smoothing(x, y, n):
    ...
    y_ma = moving_average(
        interp1d(x, y, kind='linear', bounds_error=False, fill_value=(y[0], y[-1]))(x_extended),
        n
    )
    return x_ma, y_ma
...
def process_interval(df: pd.DataFrame):
    ...
    for i in range(n_fittings):
        ...
        last_end = -1
        for start, end in filtering.smooth_intervals(filtering.intervals_from_mask(indicator_mask)):
            if start>last_end:
                last_end, corrected_H = barometric_height_correction(t, H, P, indicator_mask, start, end)
...
def barometric_height_correction(t, H, P, mask, imin, imax):
    ...
    def barometric_fit(P, H, weights):
        P0_est = P_points[0]
        a_est = 8400
        popt, _ = curve_fit(barometric_func, P, H, p0=[P0_est, a_est], sigma=weights**(-2))
        return popt
