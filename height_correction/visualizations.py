...
def plot_filtering_masking(df, fig=None):
    ...
    ax_thresh.plot(t_ind, indicator, color='tab:purple', label='$MA(|H^{(HPF)}_{GPS} - H^{(HPF)}_{bar}|)$')
    ax_thresh.axhline(INDICATOR_THRESHOLD, color='tab:red', label='Correction threshold')
    ax_thresh.legend()
